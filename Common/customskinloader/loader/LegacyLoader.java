	...
	public UserProfile loadProfile(SkinSiteProfile ssp, GameProfile gameProfile) throws Exception {
		...
		if(StringUtils.isNoneEmpty(ssp.skin)){
			...
			if(HttpUtil0.isLocal(ssp.skin)){
				...
				HttpResponce responce=HttpRequestUtil.makeHttpRequest(new HttpRequest(skin).setUserAgent(ssp.userAgent).setCheckPNG(ssp.checkPNG!=null&&ssp.checkPNG!=false).setLoadContent(false).setCacheTime(60));
				if(responce.success)
		...
		if(StringUtils.isNoneEmpty(ssp.cape)){
			...
			if(HttpUtil0.isLocal(ssp.cape)){
				...
				HttpResponce responce=HttpRequestUtil.makeHttpRequest(new HttpRequest(cape).setUserAgent(ssp.userAgent).setCheckPNG(ssp.checkPNG!=null&&ssp.checkPNG!=false).setLoadContent(false).setCacheTime(60));
				if(responce.success)
		...
		if(ModelManager0.isElytraSupported()&&StringUtils.isNoneEmpty(ssp.elytra)){
			...
			if(HttpUtil0.isLocal(ssp.elytra)){
				...
				HttpResponce responce=HttpRequestUtil.makeHttpRequest(new HttpRequest(elytra).setUserAgent(ssp.userAgent).setCheckPNG(ssp.checkPNG!=null&&ssp.checkPNG!=false).setLoadContent(false).setCacheTime(60));
				if(responce.success)
