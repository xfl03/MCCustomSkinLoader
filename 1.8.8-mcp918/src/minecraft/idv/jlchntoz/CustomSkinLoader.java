...
public class CustomSkinLoader {
	...
	private InputStream getStream(String URL, Boolean CheckPNG) {
		...
		if(cacheFile.exists()&&cacheFile.length()>1){
			alreadyCached=true;
			logger.info("Cache File Found (LastModified: "+cacheFile.lastModified()+")");
		}
		try {
			URL U = new URL(URL);
			...
			if (respcode != 4 && respcode != 5) { // Successful to get skin.
				logger.info("Successfully Get Resource (LastModified: "+cacheFile.lastModified()+")");
		...
		try{//Save to Local Skin File
			...
			try {
				...
				if(to.exists()){
					...
					logger.info("LastModified : "+to.lastModified());
				}
			} catch (IOException e) {
				logger.warning(e.getMessage());
			}
