...
public class ImageBufferDownload implements IImageBuffer
{
    ...
    public BufferedImage parseUserSkin(BufferedImage image)
    {
        ...
        else
        {
            ...
            if(!customskinloader.CustomSkinLoader.config.enableTransparentSkin){
                this.setAreaOpaque( 0 * ratio,  0 * ratio, 32 * ratio, 16 * ratio);

                if (flag)
                {
                    this.doTransparencyHack(32 * ratio, 0, 64 * ratio, 32 * ratio);
                }

                this.setAreaOpaque( 0 * ratio, 16 * ratio, 64 * ratio, 32 * ratio);
